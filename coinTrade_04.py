...
while True:
    try:
        ...
        price_rate = 0
        if price > 0:
            price_rate = round((price - df.iloc[loop -1]['close']) / price * 100,3)
        #현재 매입잔고의 수익률
        expect_rate = 0
        ...
        # print(df[loop-20:loop]['rate'].sum())
        ...
        if sell_pos == "Y" and df.iloc[loop -2]['rate'] > 1 and price_rate < -0.5 :
            if expect_rate > 0.5 :
        ...
        # 매도전략 3 : 상승 10연승 중에 하방 시그널이 크게 잡혔을때
        if sell_pos == "Y" and df.iloc[loop -3]['v_strong'] > 10 and df.iloc[loop -2]['rate'] < -0.5 : 
        ...
        if sell_pos == "Y" and df.iloc[loop -3]['v_strong'] > 3 and df.iloc[loop -2]['v_pm'] < 0 and df.iloc[loop -2]['v_rsi_real'] >= 65 : 
            if expect_rate > 0.1:
        ...
        if sell_pos == "Y" and expect_rate < -1.5 :
            sign_sell_from_type = 8
            sign_sell_rate = 1
            sign_action = "SELL"
        ...
        if df.iloc[loop -1]['v_pm'] > 0 and df.iloc[loop -2]['v_pm'] > 0 and df.iloc[loop -4]['v_rsi_real'] <= 40 :
        ...
        if sell_pos == "Y" and sign_action == "SELL" :
            ...
            if 1 == 1:                    
                ...
                if medo_amt > 5100 :
                    medo_profit = round((medo_amt - (item_qty * item_avg_buy_price)) / (item_qty * item_avg_buy_price) * 100, 3)
        ...
        if sell_pos == "Y" and sign_action_add == "BUY" and buy_add_pos == "Y" :
            ...
            if 1 == 1:
                ...
                add_qty = round(mesu_amt / price, 9) 
                ...
                data = "added buying (%d) : buy_price :  %f , mesu_2_amt : %f  \n" % (deal_add_count, price, mesu_2_amt)
                file.write(data)
                file.flush()
